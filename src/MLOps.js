...
export const MLOps = ({ srvurl, setError, setActionButton, setSpin }) => {
  ...
  React.useEffect(() => {
    ...
    // fetch app list
    fetch(`${srvurl}/mlapps`, { mode: "cors" }).then((res) =>
      res
        .json()
      ...
      .then((config) => {
        if (config) {
          ...
          if (remember && credentials)
            localStorage.setItem("ezmeral", JSON.stringify(credentials));
  ...
  const k8sc_list = () => {
    getData("platform/list").then((data) => {
  ...
  const appOp = (op, tenant, app) => {
    ...
    getData(url, { app, tenant }).then((data) =>
      setAlert({
        title:
          op.toUpperCase() +
          ' "' +
          ...
          <Box fill="horizontal">
            <Box>
              ...
              <DataTable
                ...
                border="bottom"
                onClickRow={({ datum }) =>
                  setPlatform((old) => ({ ...old, tenant: datum }))
                }
                columns={[
                  {
                    property: "status",
                    render: (t) =>
                      t.status === "ready" ? (
                        ...
                        <Tip content="Create Tenant">
                          <AddCircle />
                        </Tip>
                      ) : (
                        <Tip content={t.status}>
                          <StatusCritical />
                        </Tip>
                      ),
                  },
